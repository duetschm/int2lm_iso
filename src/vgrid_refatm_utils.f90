...
! Global Parameters
  INTEGER (KIND=iintegers), PARAMETER     ::                       &
    khmax = 199         ! max. number of vertical coordinate parameters
                        ! according to grib.
...
INTEGER (KIND=iintegers)   :: k
REAL (KIND=ireals)         :: zpxx, zn, zt00, zp0sl, zt0sl, zdelta_t, zh_scal
REAL (KIND=ireals)         :: zak(ke+1), zbk(ke+1), zbk2(ke+1), zhfl(ie,je)
...
!   ENDDO
  ...
  CASE ( 2, 3, 4 )
    ...
    ELSEIF (vc_type%ivctype == 3 .OR. vc_type%ivctype == 4) THEN
      ...
      ! Vertical grid specified in terms of hhl
      ! switch between original and revised SLEVE coordinate
      zn = 1.00_ireals
  ...
  IF (COUNT(hhl(:,:,1:ke) - hhl(:,:,2:ke+1) <= 0.0_ireals) > 0) THEN
    WRITE(*,*) "ERROR: negative dz encountered reference_atmosphere_2!"
  ENDIF
...
INTEGER (KIND=iintegers)   :: k
REAL (KIND=ireals)         :: zn, zgdrt, ztdbe, zbetf, zpxx, zth00, zp00, zc_p,   &
...
!   ENDDO
  ...
  ! Basic consistency check for hhl
  IF (COUNT(hhl(:,:,1:ke) - hhl(:,:,2:ke+1) <= 0.0_ireals) > 0) THEN
    WRITE(*,*) "ERROR: negative dz encountered in reference_atmosphere_BVconst!"
  ENDIF
