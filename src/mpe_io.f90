...
INTEGER , parameter    :: BUFF_length = 5000000  ! value for dwdlib...
...
INTEGER                             :: new_len
! <fuo_lockfile
CHARACTER    (LEN= dat_len+7)       :: datname_new
! fuo_lockfile>
INTEGER      (KIND=gribc_kind)      :: nudatc     &! local file descriptor
...
! <fuo_lockfile
   ! rename to lockfile
   datname_new = datname_new(1:LEN_TRIM(datname_new)) // '.lck'
   new_len = new_len + 4
! fuo_lockfile>
...
CHARACTER (LEN=250)          ::  & ! file name
  yname
INTEGER  (KIND= gribc_kind ) :: iexi, iopn, irm ! arguments for cinquire
! fuo_lockfile>
!--------------------------------------------------------------------------
!
! Error checks     :
